...
Let's go back to the transporation sector and evaluate family owned businesses. We can add one more endpoint modification to look at family business own.

For demonstration purposes, lets again pull family business data down from the CB endpoint again, this time using the veteran status modififier, and industry code. We will filter the data to remove the high level aggregation rows present in this data (e.g. stratification totals, classifiable, unclassifiable counts)
...
```
Let's see how veterans owners are presented across family business classifications.Summarize the frequency of ownership by Vet Status and Family Business status.
...
pollster::topline(CB_48Data, variable =VET_GROUP,weight=FIRMPDEMP, valid_pct = FALSE,cum_pct =FALSE)
```
