...
The `pollster` package was included to facilitate cross tab summaries of aggregated survey data. Finally, the `gghalves` package was included to create addtional graphing styles.  Select functions from `knitr` are used to create table output.  
__Libraries:__  
`library(tidyverse)`  
`library(httr)`  
`library(jsonlite)`  
...
__CB Data__   
`CBPath`  - Creates query path.  Four user query parameters are allowed - NCAICS code, Survey Question, Veteran Status, and Owner Sex.
`CBData`   - Retreives data from the Characteristics of Businesses endpoint.  
...
__NAICS2017-__   The North American Industry Classificatoin System (NAICS) industry code is the primary subsetting variable for both datasets. It is from the 2017 NAICS table. The arguement for this queray parameter is labeled `industryCode=`. This value can take on any 2-4 digit representation of the NAICS table, using a "*" as a wildcard.

__VETERAN STATUS-__ A categorical variable that returns the status of the business owner as a veteran.  The query parameter for this function arguement is `vets=`, and cand take on the value, of vet, nonvet, both, all or a wildcard. 
...
CS_AllVets<-CSData(vets = "all", industryCode = "*")
...
```
To explore the overall presence of veteran owned firms across 
```{r RQ1Summaries  }
...
ggplot(BySector,aes(Firms, SECTOR)) +
